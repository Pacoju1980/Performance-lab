...
const metricDefs={
 ...
 bodyFat:{name:'Grasa corporal',icon:'◒',unit:'%',clinical:true,ref:p=>'La grasa corporal se interpreta mejor por tendencia; una medición BIA aislada no es diagnóstica.',good:(v,p)=>p?.sex==='female'?(v<33?'green':v<38?'yellow':'red'):(v<25?'green':v<30?'yellow':'red')},
 skeletalMuscle:{name:'Músculo esquelético',icon:'≋',unit:'kg',clinical:false},
 muscleMass:{name:'Masa muscular',icon:'◔',unit:'kg',clinical:false},
 bodyWater:{name:'Agua corporal',icon:'◌',unit:'%',clinical:false},
 bodyProtein:{name:'Proteína corporal',icon:'◎',unit:'%',clinical:false},
 boneMass:{name:'Masa ósea',icon:'◇',unit:'kg',clinical:false},
 bmr:{name:'Metabolismo basal',icon:'∿',unit:'kcal',clinical:false}
};
...
function baselineMedian(key){return median(baselineDates().map(d=>state.daily[d]?.metrics?.[key]))}
function baselineCount(key){return baselineDates().map(d=>state.daily[d]?.metrics?.[key]).filter(Number.isFinite).length}
function baselineDeltaPct(key,v){let b=baselineMedian(key);return Number.isFinite(v)&&Number.isFinite(b)&&b!==0?(v-b)/b*100:null}
function personalBaselineReady(){return baselineDates().length>=15}
function metricValue(date,key){let d=state.daily[date];if(key==='energy')return d?.checkins?.morning?.energy??d?.checkins?.pre?.energy??null;if(key==='stress'&&!Number.isFinite(d?.metrics?.stress)){return mean([d?.metrics?.stressMorning,d?.metrics?.stressNoon,d?.metrics?.stressPreWorkout])}if(['bmi','waistHeight','weight','waist','bodyFat','skeletalMuscle','muscleMass','bodyWater','bodyProtein','boneMass','bmr'].includes(key)){let a=[...state.anthro].filter(x=>x.date<=date).sort((x,y)=>y.date.localeCompare(x.date))[0]; if(!a)return null;if(key==='waistHeight')return a.waist&&state.profile?.height?a.waist/(state.profile.height):null;return a[key]??null}return d?.metrics?.[key]??null}
function periodDates(period){return daysBack(period==='day'?1:period==='week'?7:30)}
function seriesFor(key,period=ui.period){
 const anthroKeys=['weight','bmi','bodyFat','skeletalMuscle','muscleMass','bodyWater','bodyProtein','boneMass','bmr','waist'];
 if(anthroKeys.includes(key)){
   const ds=new Set(periodDates(period));
   return [...state.anthro].filter(a=>ds.has(a.date)&&Number.isFinite(a[key])).sort((a,b)=>a.date.localeCompare(b.date)).map(a=>({date:a.date,value:Number(a[key])}));
 }
 let ds=periodDates(period);return ds.map(d=>({date:d,value:metricValue(d,key)})).filter(x=>Number.isFinite(x.value))
}
...
function physioAlertFor(key,date=iso()){
 const v=metricValue(date,key),b=baselineMedian(key),delta=baselineDeltaPct(key,v),ready=personalBaselineReady()&&baselineCount(key)>=8;
 if(!Number.isFinite(v))return {key,level:'none',color:'gray',value:null,reasons:['Sin dato disponible']};
 let level='ok',reasons=[];
 const worsen=(lvl,msg)=>{const rank={ok:0,info:1,watch:2,alert:3};if(rank[lvl]>rank[level])level=lvl;reasons.push(msg)};
 if(key==='spo2'){
   const alt=Number(state.profile?.altitude||0);
   if(alt>=1500){if(v<90)worsen('alert','SpO₂ por debajo de 90%');else if(v<92)worsen('watch','SpO₂ por debajo del intervalo esperado para la altitud');}
   else {if(v<92)worsen('alert','SpO₂ claramente baja');else if(v<95)worsen('watch','SpO₂ por debajo del intervalo habitual');}
   if(ready&&Number.isFinite(delta)){if(delta<=-5)worsen('alert',`↓ ${Math.abs(delta).toFixed(1)}% vs tu basal`);else if(delta<=-2.5)worsen('watch',`↓ ${Math.abs(delta).toFixed(1)}% vs tu basal`);}
 }
 if(key==='restingHR'){
   if(v>100||v<45)worsen('alert','FC en reposo fuera del intervalo clínico general');else if(v>90||v<50)worsen('watch','FC en reposo en zona de vigilancia');
   if(ready&&Number.isFinite(delta)){if(delta>=20)worsen('alert',`↑ ${delta.toFixed(1)}% vs tu basal`);else if(delta>=10)worsen('watch',`↑ ${delta.toFixed(1)}% vs tu basal`);}
 }
 if(key==='respiratoryRate'){
   if(v>22||v<10)worsen('alert','Frecuencia respiratoria fuera del intervalo de vigilancia');else if(v>20||v<12)worsen('watch','Frecuencia respiratoria fuera del intervalo habitual');
   if(ready&&Number.isFinite(delta)){if(Math.abs(delta)>=25)worsen('alert',`${delta>=0?'↑':'↓'} ${Math.abs(delta).toFixed(1)}% vs tu basal`);else if(Math.abs(delta)>=15)worsen('watch',`${delta>=0?'↑':'↓'} ${Math.abs(delta).toFixed(1)}% vs tu basal`);}
 }
 if(key==='sleepHours'){
   if(v<5)worsen('alert','Sueño total menor de 5 h');else if(v<6)worsen('watch','Sueño total menor de 6 h');
   if(ready&&Number.isFinite(delta)){if(delta<=-30)worsen('alert',`↓ ${Math.abs(delta).toFixed(0)}% vs tu basal`);else if(delta<=-15)worsen('watch',`↓ ${Math.abs(delta).toFixed(0)}% vs tu basal`);}
 }
 if(key==='hrv'){
   if(ready&&Number.isFinite(delta)){if(delta<=-30)worsen('alert',`VFC ↓ ${Math.abs(delta).toFixed(0)}% vs basal`);else if(delta<=-15)worsen('watch',`VFC ↓ ${Math.abs(delta).toFixed(0)}% vs basal`);}
   else if(Number.isFinite(v))worsen('info','La VFC se interpreta principalmente contra tu propio basal');
 }
 const map={ok:['green','Dentro de referencia'],info:['blue','Contexto'],watch:['yellow','Atención'],alert:['red','Alerta']};
 return {key,level,color:map[level][0],label:map[level][1],value:v,baseline:b,delta,reasons:reasons.length?reasons:['Sin desviaciones relevantes detectadas']};
}
function physiologicalAlerts(date=iso()){
 const keys=['spo2','restingHR','respiratoryRate','sleepHours','hrv'];
 const rows=keys.map(k=>physioAlertFor(k,date)).filter(x=>x.level!=='none'&&x.level!=='ok');
 const rank={alert:3,watch:2,info:1};return rows.sort((a,b)=>(rank[b.level]||0)-(rank[a.level]||0));
}
function qualityFromAlert(key,date=iso()){
 const a=physioAlertFor(key,date);
 if(a.level==='alert')return 25;if(a.level==='watch')return 60;if(a.level==='info')return 75;return 90;
}
function stressQuality(date=iso()){
 const v=metricValue(date,'stress');if(!Number.isFinite(v))return null;
 let q=v<40?90:v<60?72:v<80?48:25;
 let d=baselineDeltaPct('stress',v);if(personalBaselineReady()&&Number.isFinite(d)&&d>25)q=Math.min(q,55);
 return q;
}
function scoreHealth(date=iso()){
 const weights={spo2:20,restingHR:15,sleepHours:15,respiratoryRate:10,hrv:10,stress:10},parts=[];
 for(const [k,w] of Object.entries(weights)){const v=metricValue(date,k);if(!Number.isFinite(v))continue;const q=k==='stress'?stressQuality(date):qualityFromAlert(k,date);if(Number.isFinite(q))parts.push({k,w,q})}
 const bc=bodyCompositionStatus(date),la=latestAnthro(),bodyAge=la?Math.floor((new Date(date+'T12:00:00')-new Date(la.date+'T12:00:00'))/86400000):Infinity,bodyCurrent=la&&bodyAge<=10&&Number.isFinite(la.bmi);
 if(Number.isFinite(bc.score))parts.push({k:'bodyComposition',w:20,q:bc.score});
 if(parts.length<2)return {score:null,color:'gray',label:'Datos insuficientes',parts,factors:parts.length,bodyCurrent};
 const ws=parts.reduce((a,x)=>a+x.w,0);let s=Math.round(parts.reduce((a,x)=>a+x.q*x.w,0)/ws),vitalAlerts=['spo2','restingHR','respiratoryRate'].map(k=>physioAlertFor(k,date));
 if(vitalAlerts.some(x=>x.level==='alert'))s=Math.min(s,44);if(vitalAlerts.filter(x=>x.level==='watch').length>=2)s=Math.min(s,59);if(Number.isFinite(bc.score)&&bc.score<45)s=Math.min(s,59);if(!bodyCurrent)s=Math.min(s,79);
 const color=s>=80?'green':s>=60?'yellow':'red',label=!bodyCurrent&&s>=60?'Composición semanal pendiente':s>=80?'Estable':s>=60?'Vigilar':'Alerta';return {score:s,color,label,parts,factors:parts.length,bodyCurrent}
}
function recentLoadQuality(date=iso()){
 const d=new Date(date+'T12:00:00'),vals=[];
 for(let i=1;i<=3;i++){let x=new Date(d);x.setDate(x.getDate()-i);vals.push(Number(metricValue(iso(x),'workoutMinutes'))||0)}
 const yesterday=vals[0],sum=vals.reduce((a,b)=>a+b,0);
 let q=90;if(yesterday>100)q=50;else if(yesterday>75)q=65;else if(yesterday>0)q=80;
 if(sum>220)q=Math.min(q,55);else if(sum>160)q=Math.min(q,70);
 return q;
}
function sleepReadinessQuality(date=iso()){
 const v=metricValue(date,'sleepHours');if(!Number.isFinite(v))return null;
 let q=v>=7.5?95:v>=7?90:v>=6.5?78:v>=6?65:v>=5?45:25;
 let d=baselineDeltaPct('sleepHours',v);if(personalBaselineReady()&&Number.isFinite(d)){if(d<=-30)q=Math.min(q,25);else if(d<=-15)q=Math.min(q,55)}
 const ss=metricValue(date,'sleepScore');if(Number.isFinite(ss))q=.75*q+.25*clamp(ss);
 return q;
}
function autonomicQuality(date=iso()){
 let parts=[];
 const hrv=metricValue(date,'hrv'),rhr=metricValue(date,'restingHR');
 if(Number.isFinite(hrv)){let d=baselineDeltaPct('hrv',hrv),q=Number.isFinite(d)?clamp(80+d*1.2):75;parts.push({w:15,q})}
 if(Number.isFinite(rhr)){let d=baselineDeltaPct('restingHR',rhr),q=Number.isFinite(d)?clamp(88-Math.max(0,d)*2.2):qualityFromAlert('restingHR',date);parts.push({w:10,q})}
 if(!parts.length)return null;return parts.reduce((a,x)=>a+x.w*x.q,0)/parts.reduce((a,x)=>a+x.w,0)
}
function scoreTraining(date=iso()){
 const components=[
   {k:'sleepRecovery',name:'Sueño y recuperación nocturna',w:35,q:sleepReadinessQuality(date)},
   {k:'recentLoad',name:'Carga y recuperación muscular reciente',w:25,q:recentLoadQuality(date)},
   {k:'autonomic',name:'VFC + FC en reposo',w:25,q:autonomicQuality(date)},
   {k:'stressLoad',name:'Estrés fisiológico',w:15,q:stressQuality(date)}
 ].filter(x=>Number.isFinite(x.q));
 if(components.length<2)return {score:null,color:'gray',label:'Datos insuficientes',parts:components};
 const ws=components.reduce((a,x)=>a+x.w,0);let sc=Math.round(components.reduce((a,x)=>a+x.q*x.w,0)/ws);
 const hard=physiologicalAlerts(date).filter(x=>x.level==='alert'&&['spo2','restingHR','respiratoryRate'].includes(x.key));
 if(hard.length)sc=Math.min(sc,49);
 const color=sc>=80?'green':sc>=60?'yellow':'red';
 return {score:sc,color,label:sc>=80?'Condiciones favorables':sc>=60?'Ajustar según respuesta':'Priorizar recuperación',parts:components,zeppContrast:metricValue(date,'zeppReadiness')};
}
...
function applyDayImport(item,defaultSource,generatedAt){
 ...
 const bodyKeys=['weight','bmi','bodyFat','skeletalMuscle','muscleMass','bodyWater','bodyProtein','boneMass','bmr'];
 if(bodyKeys.some(k=>Number.isFinite(Number(metrics[k])))){
   let a=state.anthro.find(x=>x.date===d)||{date:d};
   for(const k of bodyKeys)if(Number.isFinite(Number(metrics[k])))a[k]=Number(metrics[k]);
...
function applyShortcutNamedVectors(payload,source,generatedAt){
 const specs=[
  ...
  ['bmi',['BMI','IMC'],['BMIFechas','IMCFechas']],
  ['bodyFat',['BodyFat','GrasaCorporal','Grasa'],['BodyFatFechas','GrasaCorporalFechas','GrasaFechas']],
  ['skeletalMuscle',['SkeletalMuscle','MusculoEsqueletico','MúsculoEsquelético'],['SkeletalMuscleFechas','MusculoEsqueleticoFechas','MúsculoEsqueléticoFechas']],
  ['muscleMass',['MuscleMass','MasaMuscular','Musculo'],['MuscleMassFechas','MasaMuscularFechas','MusculoFechas']],
  ['bodyWater',['BodyWater','AguaCorporal','Agua'],['BodyWaterFechas','AguaCorporalFechas','AguaFechas']],
  ['bodyProtein',['BodyProtein','ProteinaCorporal','ProteínaCorporal','Proteina'],['BodyProteinFechas','ProteinaCorporalFechas','ProteínaCorporalFechas','ProteinaFechas']],
  ['boneMass',['BoneMass','MasaOsea','MasaÓsea'],['BoneMassFechas','MasaOseaFechas','MasaÓseaFechas']],
  ['bmr',['BMR','MetabolismoBasal','Metabolismo'],['BMRFechas','MetabolismoBasalFechas','MetabolismoFechas']]
 ];
...
function applyZeppScalars(payload,source,generatedAt){
 const fields=[
 ['hrv',['HRV','VFC','VariabilidadCardiaca']],['stress',['Stress','Estres','Estrés']],['pai',['PAI']],
 ['sleepScore',['SleepScore','PuntuacionSueno','PuntuaciónSueño','PuntuacionDeSueno']],
 ['zeppReadiness',['Readiness','Recovery','Recuperacion','Recuperación','ZeppReadiness']],
 ['weight',['Weight','Peso']],['bmi',['BMI','IMC']],['bodyFat',['BodyFat','GrasaCorporal','Grasa']],
 ['skeletalMuscle',['SkeletalMuscle','MusculoEsqueletico','MúsculoEsquelético']],
 ['muscleMass',['MuscleMass','MasaMuscular','Musculo']],['bodyWater',['BodyWater','AguaCorporal','Agua']],
 ['bodyProtein',['BodyProtein','ProteinaCorporal','ProteínaCorporal','Proteina']],
 ['boneMass',['BoneMass','MasaOsea','MasaÓsea']],['bmr',['BMR','MetabolismoBasal','Metabolismo']]
 ];
...
function metricSourceLabel(key,date=iso()){
 const anthroKeys=['weight','bmi','bodyFat','skeletalMuscle','muscleMass','bodyWater','bodyProtein','boneMass','bmr','waist'];
 if(anthroKeys.includes(key)){let a=[...state.anthro].filter(x=>x.date<=date).sort((x,y)=>y.date.localeCompare(x.date))[0];return a?.source||'Zepp / antropometría'}
 const r=state.daily[date];if(!r)return 'Sin fuente';if(['hrv','stress','pai','sleepScore','zeppReadiness'].includes(key))return r.sources?.find(x=>/zepp/i.test(x))||r.sources?.at(-1)||'Zepp/manual';return r.sources?.find(x=>/apple|salud|health/i.test(x))||r.sources?.at(-1)||'Apple Salud'
}
...
function zeppCoverage(){const core=['hrv','stress','pai','sleepScore','zeppReadiness'],body=['bmi','bodyFat','skeletalMuscle','muscleMass','bodyWater','bodyProtein','boneMass','bmr'];let n=core.filter(k=>Number.isFinite(metricValue(iso(),k))).length,bodyN=body.filter(k=>Number.isFinite(metricValue(iso(),k))).length;return {n,total:core.length,keys:core,bodyN,bodyTotal:body.length,body}}
...
function alertIcon(level){return level==='alert'?'!':level==='watch'?'△':'i'}
function physiologicalAlertPanel(){
 const rows=physiologicalAlerts(),bc=bodyCompositionStatus(),bcRow=Number.isFinite(bc.score)&&bc.score<80?`<div class="alert-row ${bc.color}"><div class="alert-symbol">◇</div><div><strong>Composición corporal · ${bc.label}</strong><small>Evaluación semanal obligatoria: IMC + grasa corporal + cintura/altura cuando están disponibles.</small></div></div>`:'';
 if(!rows.length&&!bcRow)return `<div class="alert-panel all-clear"><div class="alert-head"><span>✓</span><div><strong>Sin alertas fisiológicas</strong><small>Los datos disponibles no muestran desviaciones relevantes hoy.</small></div></div></div>`;
 return `<div class="alert-panel"><div class="alert-panel-title"><div><strong>Alertas fisiológicas</strong><small>Basal personal + referencia clínica contextual + composición corporal semanal.</small></div></div>${rows.map(a=>`<div class="alert-row ${a.color}"><div class="alert-symbol">${alertIcon(a.level)}</div><div><strong>${metricDefs[a.key]?.name||a.key} · ${a.label}</strong><small>${a.reasons.map(esc).join(' · ')}</small>${Number.isFinite(a.baseline)?`<em>Actual ${formatMetricValue(a.key,a.value)} · basal ${formatMetricValue(a.key,a.baseline)}</em>`:''}</div></div>`).join('')}${bcRow}<p class="alert-foot">Una alerta no es un diagnóstico. La composición corporal de Zepp es una estimación BIA y debe interpretarse por tendencia, en condiciones de medición comparables.</p></div>`
}

function previousAnthro(){const rows=[...state.anthro].sort((a,b)=>b.date.localeCompare(a.date));return rows[1]||null}
function bodyCompositionStatus(date=iso()){
 const a=[...state.anthro].filter(x=>x.date<=date).sort((x,y)=>y.date.localeCompare(x.date))[0];if(!a)return {score:null,color:'gray',label:'Sin medición',parts:[]};
 const parts=[],add=(k,w,q)=>{if(Number.isFinite(q))parts.push({k,w,q})};
 if(Number.isFinite(a.bmi))add('bmi',40,a.bmi<25?95:a.bmi<30?65:a.bmi<35?35:20);
 if(Number.isFinite(a.bodyFat)){const male=state.profile?.sex!=='female';let q=male?(a.bodyFat<20?95:a.bodyFat<25?80:a.bodyFat<30?55:30):(a.bodyFat<28?95:a.bodyFat<33?80:a.bodyFat<38?55:30);add('bodyFat',35,q)}
 if(Number.isFinite(a.waist)&&Number.isFinite(state.profile?.height)){const wh=a.waist/state.profile.height;add('waistHeight',25,wh<.5?95:wh<.6?60:30)}
 if(!parts.length)return {score:null,color:'gray',label:'Datos insuficientes',parts};
 const ws=parts.reduce((s,x)=>s+x.w,0),score=Math.round(parts.reduce((s,x)=>s+x.w*x.q,0)/ws);return {score,color:score>=80?'green':score>=60?'yellow':'red',label:score>=80?'Favorable':score>=60?'Vigilar':'Fuera de objetivo',parts}
}
function bodyCompositionProgress(){
 const latest=latestAnthro(),prev=previousAnthro();if(!latest)return null;const delta=k=>prev&&Number.isFinite(latest[k])&&Number.isFinite(prev[k])?latest[k]-prev[k]:null;
 return {date:latest.date,previousDate:prev?.date||null,weight:latest.weight??null,bmi:latest.bmi??null,bodyFat:latest.bodyFat??null,skeletalMuscle:latest.skeletalMuscle??null,muscleMass:latest.muscleMass??null,bodyWater:latest.bodyWater??null,bodyProtein:latest.bodyProtein??null,boneMass:latest.boneMass??null,bmr:latest.bmr??null,dWeight:delta('weight'),dBmi:delta('bmi'),dBodyFat:delta('bodyFat'),dSkeletalMuscle:delta('skeletalMuscle'),dMuscleMass:delta('muscleMass')}
}
function bodyCompositionPanel(){
 const p=bodyCompositionProgress(),s=bodyCompositionStatus();if(!p)return `<div class="card bodycomp-card"><div class="empty">Aún no hay composición corporal semanal. Impórtala desde Zepp o regístrala manualmente.</div></div>`;
 const delta=(v,unit='')=>Number.isFinite(v)?`${v>0?'+':''}${v.toFixed(1)}${unit}`:'—',rows=[['IMC',p.bmi,delta(p.dBmi),'kg/m²','bmi'],['Grasa corporal',p.bodyFat,delta(p.dBodyFat,'%'),'%', 'bodyFat'],['Músculo esquelético',p.skeletalMuscle,delta(p.dSkeletalMuscle,' kg'),'kg','skeletalMuscle'],['Masa muscular',p.muscleMass,delta(p.dMuscleMass,' kg'),'kg','muscleMass'],['Peso',p.weight,delta(p.dWeight,' kg'),'kg','weight']];
 return `<div class="card bodycomp-card"><div class="bodycomp-head"><div><span>Composición corporal semanal</span><strong>${fmtDate(p.date)}</strong></div><span class="chip ${s.color}">${s.label}</span></div><div class="bodycomp-kpis">${rows.map(([l,v,d,u,k])=>`<button data-metric="${k}" class="bodycomp-kpi"><span>${l}</span><strong>${Number.isFinite(v)?round(v):'—'} <small>${Number.isFinite(v)?u:''}</small></strong><em>${d}</em></button>`).join('')}</div><div class="bodycomp-secondary">${[['Agua',p.bodyWater,'%'],['Proteína',p.bodyProtein,'%'],['Masa ósea',p.boneMass,'kg'],['Metabolismo basal',p.bmr,'kcal']].map(([l,v,u])=>`<span><b>${l}</b>${Number.isFinite(v)?round(v)+' '+u:'—'}</span>`).join('')}</div><p class="note">El IMC es obligatorio en Salud y seguimiento semanal. Para hipertrofia no se interpreta solo: se contrasta con grasa, músculo y perímetros porque el aumento de músculo también puede elevar el IMC.</p></div>`
}

function weeklyHypertrophy(){const ds=daysBack(7),target=Number(state.settings?.weeklySessionTarget||3);const workoutDays=ds.filter(d=>Number.isFinite(metricValue(d,'workoutMinutes'))),completed=workoutDays.filter(d=>Number(metricValue(d,'workoutMinutes'))>0).length;const planned=ds.filter(d=>state.selectedRoutine?.[d]).length,minutes=ds.reduce((a,d)=>a+(Number(metricValue(d,'workoutMinutes'))||0),0);const adherence=workoutDays.length&&target?Math.round(completed/target*100):null;const first=[...state.anthro].sort((a,b)=>a.date.localeCompare(b.date))[0],last=latestAnthro(),prev=previousAnthro();return {completed,target,planned,minutes,adherence,hasWorkoutData:workoutDays.length>0,weight:last?.weight??null,bmi:last?.bmi??null,bodyFat:last?.bodyFat??null,skeletalMuscle:last?.skeletalMuscle??null,muscleMass:last?.muscleMass??null,weightDelta:first&&last&&Number.isFinite(first.weight)&&Number.isFinite(last.weight)?last.weight-first.weight:null,weeklyMuscleDelta:prev&&last&&Number.isFinite(prev.skeletalMuscle)&&Number.isFinite(last.skeletalMuscle)?last.skeletalMuscle-prev.skeletalMuscle:null}}
function hypertrophySummary(){let h=weeklyHypertrophy();return `<div class="hypertrophy-grid"><div class="mini-kpi"><span>Sesiones realizadas</span><strong>${h.hasWorkoutData?`${h.completed}/${h.target}`:'—'}</strong><small>${h.hasWorkoutData?'últimos 7 días':'falta importar entrenamientos'}</small></div><div class="mini-kpi"><span>IMC semanal</span><strong>${Number.isFinite(h.bmi)?h.bmi.toFixed(1):'—'}</strong><small>${Number.isFinite(h.bodyFat)?`grasa ${h.bodyFat.toFixed(1)}%`:'composición pendiente'}</small></div><div class="mini-kpi"><span>Músculo esquelético</span><strong>${Number.isFinite(h.skeletalMuscle)?h.skeletalMuscle.toFixed(1)+' kg':'—'}</strong><small>${Number.isFinite(h.weeklyMuscleDelta)?`${h.weeklyMuscleDelta>=0?'+':''}${h.weeklyMuscleDelta.toFixed(1)} kg vs medición previa`:'sin comparación semanal'}</small></div></div>`}
...
function screenToday(){let routine=currentRoutine(),zc=zeppCoverage();return `${header()}<main>${baselineBanner()}<div class="section-title"><div><h2>Hoy</h2><p>Salud, recuperación y progreso para hipertrofia.</p></div><button id="whyBtn">¿Por qué?</button></div><div class="hero-grid">${heroCard('health')}${heroCard('training')}</div>${physiologicalAlertPanel()}<div class="section-title"><div><h2>Resumen fisiológico</h2><p>Datos automáticos; toca una tarjeta para ver detalle.</p></div></div><div class="summary-list">${['sleepHours','restingHR','spo2','respiratoryRate','steps'].map(summaryMetricCard).join('')}</div><div class="section-title"><div><h2>Zepp</h2><p>VFC, estrés, PAI, sueño, recuperación y composición corporal semanal.</p></div><span class="section-badge">${zc.n}/${zc.total} · cuerpo ${zc.bodyN}/${zc.bodyTotal}</span></div><div class="summary-list compact">${['hrv','stress','pai','sleepScore','zeppReadiness'].map(summaryMetricCard).join('')}</div><div class="section-title"><div><h2>Check-in subjetivo</h2><p>Escala 1–5. Se analiza por separado y no altera el semáforo objetivo.</p></div></div><div class="checkin-grid">${checkinSummaryCard('morning')}${checkinSummaryCard('pre')}</div><div class="section-title"><div><h2>Progreso de hipertrofia</h2><p>Adherencia al plan + cambios corporales.</p></div><button data-go="progress">Ver progreso</button></div>${hypertrophySummary()}${bodyCompositionPanel()}<div class="section-title"><div><h2>Entrenamiento de hoy</h2><p>${routine?'Rutina seleccionada del manual':'Selecciona la sesión que corresponda.'}</p></div><button data-go="train">Rutinas</button></div><div class="card workout-today">${routine?`<div><span class="chip ${routine.accent}">${routine.phase}</span><h3>${routine.label}</h3><p>${routine.exercises.length} ejercicios · seguimiento de sesión global.</p></div>`:`<div class="empty">No has elegido rutina para hoy.</div>`}</div></main>${nav('today')}`}
function screenHealth(){let p=ui.period;return `${header()}<main><div class="section-title"><div><h2>Salud</h2><p>Detalle por variable y evolución personal.</p></div></div>${periodSelector(p)}<div class="section-title sub"><div><h3>Apple Salud</h3><p>Datos fisiológicos importados.</p></div></div><div class="summary-list">${['sleepHours','restingHR','spo2','respiratoryRate','steps'].map(summaryMetricCard).join('')}</div><div class="section-title sub"><div><h3>Zepp</h3><p>Métricas complementarias del Amazfit.</p></div></div><div class="summary-list">${['hrv','stress','pai','sleepScore','zeppReadiness'].map(summaryMetricCard).join('')}</div><div class="section-title sub"><div><h3>Composición corporal semanal</h3><p>IMC obligatorio + grasa, músculo y estimaciones BIA de Zepp.</p></div></div>${bodyCompositionPanel()}<div class="card chart-card"><h3>Historial de preparación · 28 días</h3><div class="heatmap">${heatmap()}</div><div class="status-strip" style="margin-top:10px"><span class="chip green">Favorable</span><span class="chip yellow">Precaución</span><span class="chip red">Alerta</span><span class="chip gray">Sin datos</span></div></div></main>${nav('health')}`}
...
function anthroSummary(){let rows=[...state.anthro].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,6);if(!rows.length)return '<div class="card empty">Aún no hay mediciones antropométricas.</div>';let l=rows[0],wh=l?.waist&&state.profile?.height?l.waist/state.profile.height:null;return `<div class="card"><div class="status-strip" style="margin-bottom:10px">${Number.isFinite(l?.bmi)?`<span class="chip ${clinicalColor('bmi',l.bmi)}">IMC* ${round(l.bmi)}</span>`:''}${Number.isFinite(l?.bodyFat)?`<span class="chip ${clinicalColor('bodyFat',l.bodyFat)}">Grasa* ${round(l.bodyFat)}%</span>`:''}${Number.isFinite(l?.waist)?`<span class="chip ${clinicalColor('waist',l.waist)}">Cintura* ${round(l.waist)} cm</span>`:''}${Number.isFinite(wh)?`<span class="chip ${clinicalColor('waistHeight',wh)}">Cintura/altura* ${wh.toFixed(2)}</span>`:''}</div><div class="table-scroll"><table class="table"><tr><th>Fecha</th><th>Peso</th><th>IMC</th><th>Grasa</th><th>Músculo esq.</th><th>Masa muscular</th></tr>${rows.map(a=>`<tr><td>${fmtDate(a.date)}</td><td>${a.weight??'—'}</td><td>${a.bmi??'—'}</td><td>${a.bodyFat??'—'}</td><td>${a.skeletalMuscle??'—'}</td><td>${a.muscleMass??'—'}</td></tr>`).join('')}</table></div><div class="clinical"><b>Interpretación:</b> IMC es un indicador obligatorio de salud y tendencia, pero el progreso de hipertrofia se decide con el conjunto: músculo esquelético, masa muscular, grasa, peso y perímetros. Los valores BIA se comparan preferentemente semana a semana en condiciones similares.</div></div>`}
...
function modalHtml(m){
 ...
 if(m.type==='why'){let h=scoreHealth(),t=scoreTraining(),sub=subjectiveScore('pre');return `<div class="modal-back"><div class="modal"><div class="modal-handle"></div><button class="close">×</button><h2>¿Por qué?</h2><div class="card flat"><h3>Salud · ${h.label}</h3><p class="note">Ponderación objetiva: SpO₂ 20%, FC en reposo 15%, sueño 15%, respiración 10%, VFC 10%, estrés 10% y composición corporal semanal 20%. Dentro de composición: IMC 40%, grasa corporal 35% y cintura/altura 25% cuando están disponibles. Sin IMC semanal vigente, Salud no puede quedar en verde completo. Una alerta vital relevante puede limitar el color global.</p><table class="table">${(h.parts||[]).map(x=>`<tr><td>${metricDefs[x.k]?.name||x.k}</td><td>${Math.round(x.q)}/100</td></tr>`).join('')||'<tr><td>Datos insuficientes</td></tr>'}</table></div><div class="card flat" style="margin-top:10px"><h3>Preparación · ${t.label}</h3><p class="note">Ponderación objetiva: sueño/recuperación nocturna 35%, carga y recuperación muscular reciente 25%, VFC + FC en reposo 25% y estrés fisiológico 15%. El readiness de Zepp se usa como contraste, no se duplica dentro del cálculo.</p><table class="table">${(t.parts||[]).map(x=>`<tr><td>${x.name||x.k}</td><td>${Math.round(x.q)}/100</td></tr>`).join('')||'<tr><td>Datos insuficientes</td></tr>'}</table><div class="comment">${recommendation(t)}</div>${Number.isFinite(sub)?`<div class="note">Check-in pre-entreno: ${sub}/100 · se muestra por separado y no altera el semáforo objetivo.</div>`:''}${Number.isFinite(t.zeppContrast)?`<div class="note">Zepp readiness: ${Math.round(t.zeppContrast)}/100 · comparación externa.</div>`:''}</div></div></div>`}
...
function zeppModal(){let m=today().metrics,a=latestAnthro()||{};return `<div class="modal-back"><div class="modal zepp-modal"><div class="modal-handle"></div><button class="close">×</button><h2>Zepp / Amazfit</h2><p class="note">Importación prioritaria: recuperación y composición corporal semanal. Los valores de composición son estimaciones BIA; conviene medirlos en condiciones similares.</p><form id="zeppForm" class="form"><div class="section-title sub"><div><h3>Recuperación</h3></div></div><div class="form-grid"><div class="field"><label>VFC / HRV (ms)</label><input name="hrv" type="number" step="0.1" value="${m.hrv??''}"></div><div class="field"><label>Estrés (0–100)</label><input name="stress" type="number" min="0" max="100" step="1" value="${m.stress??''}"></div><div class="field"><label>PAI</label><input name="pai" type="number" step="1" value="${m.pai??''}"></div><div class="field"><label>Puntuación de sueño</label><input name="sleepScore" type="number" min="0" max="100" step="1" value="${m.sleepScore??''}"></div><div class="field"><label>Recuperación / readiness</label><input name="zeppReadiness" type="number" min="0" max="100" step="1" value="${m.zeppReadiness??''}"></div></div><div class="section-title sub"><div><h3>Composición corporal semanal</h3></div></div><div class="form-grid">${[['weight','Peso (kg)'],['bmi','IMC'],['bodyFat','Grasa corporal (%)'],['skeletalMuscle','Músculo esquelético (kg)'],['muscleMass','Masa muscular (kg)'],['bodyWater','Agua corporal (%)'],['bodyProtein','Proteína corporal (%)'],['boneMass','Masa ósea (kg)'],['bmr','Metabolismo basal (kcal)']].map(([k,l])=>`<div class="field"><label>${l}</label><input name="${k}" type="number" step="0.1" value="${a[k]??''}"></div>`).join('')}<div class="field"><label>Fecha</label><input name="date" type="date" value="${iso()}"></div></div><button class="primary">Guardar datos Zepp</button></form></div></div>`}
function anthroModal(){let a=latestAnthro()||{};return `<div class="modal-back"><div class="modal"><div class="modal-handle"></div><button class="close">×</button><h2>Antropometría</h2><p class="note">Peso, IMC y composición: semanal. Perímetros: cada 15 días. Si Zepp entrega IMC, se conserva como fuente y se contrasta con el IMC calculado por altura/peso.</p><form id="anthroForm" class="form"><div class="form-grid">${[['weight','Peso (kg)'],['bmi','IMC Zepp'],['bodyFat','Grasa (%)'],['skeletalMuscle','Músculo esquelético (kg)'],['muscleMass','Masa muscular (kg)'],['bodyWater','Agua (%)'],['bodyProtein','Proteína (%)'],['boneMass','Masa ósea (kg)'],['bmr','Metabolismo basal (kcal)'],['neck','Cuello (cm)'],['shoulders','Hombros (cm)'],['chest','Pecho (cm)'],['waist','Cintura (cm)'],['hips','Cadera (cm)']].map(([k,l])=>`<div class="field"><label>${l}</label><input name="${k}" type="number" step="0.1" value="${a[k]??''}"></div>`).join('')}</div><button class="primary">Guardar medición</button></form></div></div>`}
...
function bind(){
 ...
 $('#zeppForm')?.addEventListener('submit',e=>{e.preventDefault();let f=new FormData(e.target),d=f.get('date')||iso(),r=dayObj(d),changed=false;for(const k of ['hrv','stress','pai','sleepScore','zeppReadiness']){let raw=f.get(k);if(raw!==''&&raw!==null){let v=Number(raw);if(Number.isFinite(v)){r.metrics[k]=v;changed=true}}}let a=state.anthro.find(x=>x.date===d)||{date:d};for(const k of ['weight','bmi','bodyFat','skeletalMuscle','muscleMass','bodyWater','bodyProtein','boneMass','bmr']){let raw=f.get(k);if(raw!==''&&raw!==null){let v=Number(raw);if(Number.isFinite(v)){a[k]=v;changed=true}}}if(Number.isFinite(a.weight)&&state.profile?.height&&!Number.isFinite(a.bmi))a.bmi=a.weight/((state.profile.height/100)**2);if(Object.keys(a).length>1){a.source='Zepp manual';state.anthro=state.anthro.filter(x=>x.date!==d);state.anthro.push(a)}if(changed&&!r.sources.includes('Zepp manual'))r.sources.push('Zepp manual');state.lastSync=new Date().toISOString();state.lastSyncSources=[...new Set([...(state.lastSyncSources||[]),'Zepp manual'])];save();ui.modal=null;render()});
 ...
 $('#anthroForm')?.addEventListener('submit',e=>{e.preventDefault();let f=new FormData(e.target),a={date:iso(),source:'Manual'};for(const [k,v] of f.entries())a[k]=v===''?null:Number(v);if(a.weight&&state.profile?.height&&!Number.isFinite(a.bmi))a.bmi=a.weight/((state.profile.height/100)**2);state.anthro=state.anthro.filter(x=>x.date!==a.date);state.anthro.push(a);save();ui.modal=null;render()});
