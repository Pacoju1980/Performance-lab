...
function screenToday(){let train=scoreTraining(),routine=currentRoutine();return `${header()}<main>${baselineBanner()}<div class="section-title"><div><h2>Estado de hoy</h2><p>Lectura rápida en menos de 5 segundos.</p></div><button id="whyBtn">¿Por qué?</button></div><div class="hero-grid">${heroCard('health')}${heroCard('training')}</div><div class="section-title"><div><h2>Señales automáticas</h2><p>Apple Salud + tendencia semanal. Sin rellenar datos ausentes.</p></div></div><div class="grid three">${['sleepHours','restingHR','spo2','respiratoryRate','steps','workoutMinutes'].map(metricCard).join('')}</div><div class="grid" style="margin-top:12px"><div class="card chart-card"><div class="chart-title"><div><h3>Salud</h3><strong>${scoreHealth().label}</strong></div><span class="chip ${scoreHealth().color}">${scoreHealth().score??'—'}</span></div><div class="comment">${healthRecommendation()}</div></div><div class="card chart-card"><div class="chart-title"><div><h3>Entrenamiento</h3><strong>${train.label}</strong></div><span class="chip ${train.color}">${train.score??'—'}</span></div><div class="comment">${recommendation(train)}</div></div></div><div class="section-title"><div><h2>Entrenamiento</h2><p>${routine?'Rutina seleccionada para hoy':'Selecciona manualmente la sesión del manual.'}</p></div><button data-go="train">Ver rutinas</button></div><div class="card ${routine?'selected-routine':''}">${routine?`<div class="status-strip"><span class="chip blue">${routine.phase}</span><span class="chip green">Seleccionada</span></div><h3>${routine.label}</h3><p style="color:var(--muted)">${routine.exercises.length} ejercicios · Performance Lab monitoriza la sesión; carga, repeticiones y RIR quedan fuera.</p>`:`<div class="empty">No has elegido rutina para hoy.</div>`}</div><div class="section-title"><div><h2>Check-in subjetivo</h2><p>Se compara, pero no modifica los semáforos.</p></div></div><div class="grid"><button class="card secondary" data-checkin="morning">☀️ Al despertar</button><button class="card secondary" data-checkin="pre">🏋️ Antes de entrenar</button></div></main>${nav('today')}`}
...
function syncPanel(){let q=syncQuality();let missing=q.wanted.filter(k=>!Number.isFinite(today().metrics[k])).map(k=>metricDefs[k].name);return `<div class="${q.n>=4?'ok':q.n>=2?'warning':'error'}"><b>${q.n}/${q.total} métricas automáticas disponibles hoy.</b><br>${missing.length?`Faltan: ${missing.join(', ')}.`:'Cobertura automática completa.'}</div><p style="font-size:12px;color:var(--muted)">La VFC/HRV no se incluye en el contador porque Apple Salud no la está recibiendo desde tu Amazfit. Entrenamientos, puntuación de sueño y otras métricas de Zepp requieren ampliar el Atajo o una segunda fuente.</p><p style="font-size:12px;color:var(--muted)">Última actualización: ${state.lastSync?new Date(state.lastSync).toLocaleString('es-MX'):'ninguna'}. Fuentes: ${state.lastSyncSources?.join(', ')||'—'}.</p><button class="primary" id="pasteSync">Importar paquete del Atajo</button><input type="file" id="fileSync" accept="application/json,.json" style="display:none"><button class="secondary" id="fileSyncBtn" style="margin-left:6px">Importar JSON</button>`}
...
function sleepModal(m){
 ...
 <div class="sleep-tabs"><span class="active">Fases</span><span>Cantidades</span><span>Comparaciones</span></div>
